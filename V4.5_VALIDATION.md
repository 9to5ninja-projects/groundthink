...
4. Cross-domain connections (e.g., diagnostics→training→optimization) are explicitly noted
...
|---|-------------|-----------------|----------|--------|--------------|
| V1 | CUDA Kernel Integration: Real vs Mocked | Phase 0: Tasks 0.1-0.4 | Architecture | ✅ | 2026-01-10 |
| V2 | RWKV-6 Gradient Dominance Root Cause | Phase 3: Tasks 11, 12 | Diagnostics | ✅ | 2026-01-10 |
| V3 | Mamba LR Sensitivity & Non-Linearity | Phase 3.8: Task 36 | Optimization | ✅ | 2026-01-10 |
| V4 | Tokenization as Signal Dominance Root | Phase 3.7-3.8: Tasks 34-39 | Cross-Domain | ✅ | 2026-01-10 |
| V5 | Scaling Decision & Validation Priority | Phase 3.8-4: Task 40+ | Cross-Domain | ✅ | 2026-01-10 |
| V6 | Role Anchoring Emergence & Checkpoint Strategy | Phase 4: Tasks 41-43 | Architecture/Training | 🟡 | 2026-01-10 |
| V7 | Architecture-Specific Canary Tests | Phase 3.9 & 4: Diagnostics | Diagnostics/Evaluation | 🟡 | 2026-01-10 |
| V8 | Code Data Composition & Pattern Learning | Phase 2.5 & 4: Data strategy | Data/Training | 🟡 | 2026-01-10 |
| V9 | Initialization Entropy & Component Health | Phase 1: Task 6.12, G2 Gate | Diagnostics | 🟡 | PENDING |
| V10 | Checkpoint Reproducibility Validation | Phase 2-3: Checkpoints | Training | 🟡 | PENDING |
| V11 | Performance Scaling with Model Size | Phase 3: Task 19-20 | Optimization | 🟡 | PENDING |
...
- Were mocked CUDA wrappers in `cuda_backends.py` sufficient for correctness?
...
3. Validation: `test_phase0_complete.py` verified all 3 kernels actually available:
   - `causal_conv1d`: ✓ mamba-ssm native
   - `selective_scan`: ✓ mamba-ssm native, verified in G3 mini-training
...
|------|-----------------|----------------|--------|
| Phase 0 Task 0.1 | ✅ COMPLETE | Validated ✓ | Real kernels confirmed |
| Phase 0 Task 0.2 | ✅ COMPLETE | Validated ✓ | mamba-ssm kernels working |
| Phase 0 Task 0.3 | ✅ COMPLETE | Validated ✓ | JIT compilation reliable |
| Phase 0 Task 0.4 | ✅ COMPLETE | Validated ✓ | Hybrid integration sound |
...
**Key Insight:**
Architecture problems must be diagnosed at training dynamics level, not implementation level. The kernels are correct; the fusion strategy needs adjustment (found in Phase 3.8: Task 39 → use BPE).
...
2. **Phase 1 Task 11-12:** Diagnosed as learning rate imbalance, set `mamba_lr_mult=0.5` → ratio improved to 0.7-1.3 ✓
...
6. **Phase 3.8 (Task 36):** **Critical test:** Raised `mamba_lr_mult` from 0.5 to 1.0
   - **Result:** Ratio WORSENED (0.10→0.08), loss improved (1.59→1.53)
...
|-------|------|---------|-------------|
| Phase 1 | Task 12 | Adjusted `mamba_lr_mult` | 2.0 → 0.5 |
...
- **Task 40 (BPE Benchmark):** Critical validation step → 5K+ steps on FineWeb

**Key Insight:**
Gradient dominance ≠ component malfunction. It's emergent behavior from signal properties. Char-level data favors RWKV (smoother gradients, easier optimization). BPE preserves Mamba's semantic-level learning, achieving true balance.

**Lessons Learned:**
1. Don't assume component imbalance is a learning rate problem
2. Test extremes (0.5x → 1.0x LR) to distinguish signal from tuning issues
...
- Will doubling Mamba LR (0.5 → 1.0) restore balance?
- Is there a "sweet spot" LR for Mamba independent of RWKV?

**Discovery Process:**
1. **Phase 1 Task 12:** Applied `mamba_lr_mult=0.5` (half RWKV LR) → ratio improved to 0.7-1.3
...
2. **Phase 3.8 Task 36:** Tested inverse hypothesis: `mamba_lr_mult=1.0` (equal to RWKV)
   ...
   - **Actual:** Ratio **WORSENED** (0.10→0.08), but loss **IMPROVED** (1.59→1.53)
...
|----------|------|------------------|-----------------|--------|
| Skip Task 37 (warmup) | Phase 3.8 | "Mamba needs longer warmup" | Non-linear problem, not schedule problem | ✅ Validated |
| Skip Task 38 (regularization) | Phase 3.8 | "Penalize ratio deviation" | Loss/balance Pareto frontier, not solvable with penalty | ✅ Validated |
| Pursue Task 40 (BPE) | Phase 3.8 | "Different tokenization might help" | **CONFIRMED: Transforms gradient landscape entirely** | 🔴 CRITICAL |
...
**Key Insight:**
Non-linear response to hyperparameter changes indicates deeper structural problem. When doubling a learning rate makes things worse, not better, the issue is not under-training—it's fundamentally how the loss landscape works. BPE changes the landscape; simple tuning doesn't.
...
**Category:** Cross-Domain (Diagnostics ↔ Optimization ↔ Architecture)  
**Status:** ✅ COMPLETE
...
- **Conclusion:** "Signal Dominance Confirmed" — gate takes path of least resistance

**Phase 3.8 (Task 36):** LR extremes experiment
- Higher Mamba LR (0.5→1.0) = ratio worsens
...
```
RWKV Dominance ≠ Architectural Problem
...
| 3.8 | 40 | "Validate with BPE" | 🔴 PENDING | **CRITICAL TEST** |
...
### V5: Scaling Decision & Validation Priority — Master 8M Before Scaling to 30M
...
**Discovery Process:**
1. **Phase 3.8 Cycle Complete:** All major diagnostics done (CUDA kernels ✓, component balance ✓, tokenization ✓)
...
Option A (Rush):
  ├─ Scale to 30M immediately
  ├─ Discover problems at large scale (expensive to debug)
  ├─ Cost: Wasted compute, accumulated technical debt
  └─ Risk: Unknown unknowns inherit into production

Option B (Master):
  ├─ Validate 8M model completely
  ├─ Build diagnostic tools (State Tracing Module)
  ├─ Discover problems early (cheap to fix)
  ├─ Cost: ~1-2 weeks validation overhead
  └─ Benefit: Confidence + knowledge carry forward to 30M+
```
...
|----------|-------|---------|---------------|--------------------|----------------|
| Scale to 30M? | 4 | — | "Build after Task 40" | Early scaling = risk | ⚠️ **Build diagnostics first** |
| Task 40 (BPE) | 3.8 | 40 | Validate balance | Critical blocker | 🔴 **Run before any scaling** |
| State Tracing | 4 | NEW | Optional nice-to-have | Prerequisite for confident scaling | 🔴 **Build before 30M** |
| Component interaction | 4+ | Phase 4 | Assumed working | Unknown at 8M scale | 🟡 **Test at 8M first** |
| Long-context (NIAH) | 4 | Tasks 22+ | Planned for 8M | May differ char-level vs BPE | 🟡 **Wait for Task 40 results** |

**Downstream Implications:**

**Immediate (Next 2 weeks):**
1. Complete Task 40 (BPE benchmark) → Validate component balance
2. Build State Tracing Module → Understand hidden state dynamics
3. Run 50K step extended training on 8M (Task 20) → Stability at scale
4. Document diagnostics findings → Baseline for 30M comparison
...
**Lessons Learned:**
1. **Scaling is cheaper than debugging at scale** — validate early
2. **State-based components need explicit visualization** — build tools before scaling
3. **Architecture novelty requires extra validation** — this is normal R&D
4. **Mastery scales better than assumptions** — knowledge > speed
5. **Hidden state is black box until you look inside** — State Tracing Module is required tool
...
**Principle:** Build → Validate → Fix → Validate → Scale (not Build → Scale → Validate)
...
```
Rush Path (Build → Scale → Validate):
  8M model built ✓
  30M model built ✓✓
  Validation shows state collapse issue ✗✗✗
  Fixing at 30M: weeks of development, wasted compute, rearchitecting
  
Validation-First Path (Build → Validate → Fix → Scale):
  8M model built ✓
  State Tracing shows issue early ✓✓
  Issue fixed at 8M (cheap) ✓✓
  30M builds on proven foundation ✓✓✓
```
...
**Deliverables:**
1. **State Tracing Module** — Visualize RWKV recurrence and Mamba SSM state evolution
...
3. **Statefulness Report** — Metrics summary, visualizations, initial findings
...
**Output Files:**
- `tools/state_tracing.py` — Reusable state visualization tool
- `eval/diagnostic_tests.py` — The 4 diagnostic tests
- `logs/statefulness_report_8m_baseline.md` — Week 1 findings
...
- Track: Mutual information (state_t-1 → output_t)
...
- `metrics/baseline_8m_metrics.json` — Numerical thresholds
...
- → Return to architecture redesign, not scaling

**Output:**
- `reports/8m_validation_final.md` — Complete findings + go/no-go decision
- `VALIDATION_GATE_PASS.txt` or `VALIDATION_GATE_FAIL.md` — Binary decision + reasoning
...
├── README.md — How to validate any stateful LLM
├── state_tracing/ — Core state visualization
├── diagnostics/ — 4 diagnostic tests
├── tools/ — Health monitor, coupling analyzer, information tracer
├── metrics/ — Standard metrics definitions
├── examples/ — Mamba, RWKV, RWKV+Mamba validation reports
└── benchmarks/ — Stateful benchmark suite
```
...
- MMLU, HellaSwag, Truthful QA — stateless tasks
- NIAH, InfiniteBench — test context window, not state
...
**Category:** Cross-Domain (Data ↔ Architecture ↔ Training)  
...
**Status:** 🟡 PENDING (Framework documented, validation in Phase 4)

**Unknown/Assumption at Time:**
- At what scale does "role" (persona, tone consistency) emerge in hybrid models?
- Should 8M→30M use checkpoint expansion or fresh training?
...
|-----------|----------|-----------------|------|
| 8M → 30M | Architecture unchanged, data stable, clean 8M scaling | Checkpoint expansion | Inheriting 8M limitations |
| 8M → 30M | New fusion ratio, data shift, or 8M issues | Fresh start | Wasting compute |
| 30M → 125M | Stable scaling observed | Checkpoint expansion | Masking emerging issues |
| Any scale | Using checkpoints | Run parallel fresh training (subset) | False convergence |

**Key Validation:** Role drift detection during conversations—if model can't maintain coherent persona for 3+ turns at 30M, architecture or data needs redesign.
...
**Status:** 🟡 PENDING (Implementation in Phase 3.9 Week 1)
...
**C1: State Persistence (1-turn → 10-turn)**
```
...
**Status:** 🟡 PENDING (Implementation after Phase 3.9)
...
- V2 (gradient dominance) → Task 36 experiment → V3 (LR non-linearity)

**Optimization → Architecture:**
- V3 (LR non-linearity) → Problem structural, not tuning → V4 (tokenization root cause)

**Architecture → Data:**
- V4 (tokenization affects balance) → Recommendation: use BPE

**Data → Training:**
- V7 (BPE validation) → Phase 3.8 Task 40 BPE benchmark
...
*Next entry: V6 (Component Balance Pareto frontier) — Pending experiments*
